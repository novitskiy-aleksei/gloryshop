 ...
 * jQuery iLightbox
 */
(function(g,u,H){function C(a,b){return parseInt(a.css(b),10)||0}function G(){var a=u,b="inner";"innerWidth"in u||(b="client",a=document.documentElement||document.body);return{width:a[b+"Width"],height:a[b+"Height"]}}function da(){var a=K();u.location.hash="";u.scrollTo(a.x,a.y)}function ea(a,b){a="http://lightbox.net/getSource/jsonp.php?url="+encodeURIComponent(a).replace(/!/g,"%21").replace(/'/g,"%27").replace(/\(/g,"%28").replace(/\)/g,"%29").replace(/\*/g,"%2A");g.ajax({url:a,dataType:"jsonp"}).error(function(){b(!1)});
iLCallback=function(a){b(a)}}function P(a){var b=[];g("*",a).each(function(){var a="";"none"!=g(this).css("background-image")?a=g(this).css("background-image"):"undefined"!=typeof g(this).attr("src")&&"img"==this.nodeName.toString().toLowerCase()&&(a=g(this).attr("src"));if(-1==a.indexOf("gradient"))for(var a=a.replace(/url\(\"/g,""),a=a.replace(/url\(/g,""),a=a.replace(/\"\)/g,""),a=a.replace(/\)/g,""),a=a.split(","),d=0;d<a.length;d++)if(0<a[d].length&&-1==g.inArray(a[d],b)){var e="";x.msie&&9>x.version&&
...
f[c]:f}function X(a){a=W(a,"PATHINFO_EXTENSION");a=g.isPlainObject(a)?null:a.toString().toLowerCase();return type=0<=R.image.indexOf(a)?"image":0<=R.flash.indexOf(a)?"flash":0<=R.video.indexOf(a)?"video":"iframe"}function Y(a,b){return parseInt(b/100*a)}function S(a){return(a=String(a).replace(/^\s+|\s+$/g,"").match(/^([^:\/?#]+:)?(\/\/(?:[^:@]*(?::[^:@]*)?@)?(([^:\/?#]*)(?::(\d*))?))?([^?#]*)(\?[^#]*)?(#[\s\S]*)?/))?{href:a[0]||"",protocol:a[1]||"",authority:a[2]||"",host:a[3]||"",hostname:a[4]||"",port:a[5]||
...
"accesskey");for(c in p)a=p[c],null!=a&&(d+=E(c),b+=$(c,!1));a=b+d+"> </embed>\n</object>"}return a}function I(a){a=a||location.href;return"#"+a.replace(/^[^#]*#?(.*)$/,"$1")}var R={flash:"swf",image:"bmp gif jpeg jpg png tiff tif jfif jpe",iframe:"asp aspx cgi cfm htm html jsp php pl php3 php4 php5 phtml rb rhtml shtml txt",video:"avi mov mpg mpeg movie mp4 webm ogv ogg 3gp m4v"},M=g(u),A=g(document),x,z,F,v="",D=!!("ontouchstart"in u),J=D?"itap.lightbox":"click.lightbox",ja=D?"touchstart.lightbox":
"mousedown.lightbox",ka=D?"touchend.lightbox":"mouseup.lightbox",T=D?"touchmove.lightbox":"mousemove.lightbox",aa=function(a,b,c,d){var e=this;e.options=b;e.selector=a.selector||a;e.context=a.context;e.instant=d;1>c.length?e.attachItems():e.items=c;e.vars={total:e.items.length,start:0,current:null,next:null,prev:null,BODY:g("body"),loadRequests:0,overlay:g('<div class="lightbox-overlay"></div>'),loader:g('<div class="lightbox-loader"><div></div></div>'),toolbar:g('<div class="lightbox-toolbar"></div>'),
innerToolbar:g('<div class="lightbox-inner-toolbar"></div>'),title:g('<div class="lightbox-title"></div>'),closeButton:g('<a class="lightbox-close" title="'+e.options.text.close+'"></a>'),fullScreenButton:g('<a class="lightbox-fullscreen" title="'+e.options.text.enterFullscreen+'"></a>'),innerPlayButton:g('<a class="lightbox-play" title="'+e.options.text.slideShow+'"></a>'),innerNextButton:g('<a class="lightbox-next-button" title="'+e.options.text.next+'"></a>'),innerPrevButton:g('<a class="lightbox-prev-button" title="'+
...
e.options.text.next+'"><span></span></a>'),prevButton:g('<a class="lightbox-button lightbox-prev-button" ondragstart="return false;" title="'+e.options.text.previous+'"><span></span></a>'),thumbnails:g('<div class="lightbox-thumbnails" ondragstart="return false;"><div class="lightbox-thumbnails-container"><a class="lightbox-thumbnails-dragger"></a><div class="lightbox-thumbnails-grid"></div></div></div>'),thumbs:!1,nextLock:!1,prevLock:!1,hashLock:!1,isMobile:!1,mobileMaxWidth:980,isInFullScreen:!1,
isSwipe:!1,mouseID:0,cycleID:0,isPaused:0};e.vars.hideableElements=e.vars.nextButton.add(e.vars.prevButton);e.normalizeItems();e.availPlugins();e.options.startFrom=0<e.options.startFrom&&e.options.startFrom>=e.vars.total?e.vars.total-1:e.options.startFrom;e.options.startFrom=e.options.randomStart?Math.floor(Math.random()*e.vars.total):e.options.startFrom;e.vars.start=e.options.startFrom;d?e.instantCall():e.patchItemsEvents();e.options.linkId&&(e.hashChangeHandler(),M.lightboxHashChange(function(){e.hashChangeHandler()}));
...
this.options.path.toString().toLowerCase()?this.vars.loader.stop().animate({top:"-30px"},this.options.show.speed,"easeOutCirc"):this.vars.loader.stop().animate({left:"-30px"},this.options.show.speed,"easeOutCirc")},hideLoader:function(){this.vars.loadRequests-=1;this.vars.loadRequests=0>this.vars.loadRequests?0:this.vars.loadRequests;"horizontal"==this.options.path.toString().toLowerCase()?0>=this.vars.loadRequests&&this.vars.loader.stop().animate({top:"-192px"},this.options.show.speed,"easeInCirc"):0>=this.vars.loadRequests&&
...
this.vars.start;this.vars.current=a;this.vars.next=this.items[a+1]?a+1:null;this.vars.prev=this.items[a-1]?a-1:null;this.addContents();this.patchEvents()},addContents:function(){var a=this,b=a.vars,c=a.options,d=G(),e=c.path.toString().toLowerCase();c.mobileOptimizer&&!c.innerToolbar&&(b.isMobile=d.width<=b.mobileMaxWidth);b.overlay.addClass(c.skin).hide().css({opacity:c.overlay.opacity});c.linkId&&b.overlay.attr("linkid",c.linkId);c.controls.toolbar&&(b.toolbar.addClass(c.skin).append(b.closeButton),c.controls.fullscreen&&
b.toolbar.append(b.fullScreenButton),c.controls.slideshow&&b.toolbar.append(b.innerPlayButton),1<b.total&&b.toolbar.append(b.innerPrevButton).append(b.innerNextButton));b.BODY.addClass("lightbox-noscroll").append(b.overlay).append(b.loader).append(b.holder).append(b.nextPhoto).append(b.prevPhoto);c.innerToolbar||b.BODY.append(b.toolbar);c.controls.arrows&&b.BODY.append(b.nextButton).append(b.prevButton);c.controls.thumbnail&&1<b.total&&(b.BODY.append(b.thumbnails),b.thumbnails.addClass(c.skin).addClass("lightbox-"+
e),g("div.lightbox-thumbnails-grid",b.thumbnails).empty(),b.thumbs=!0);d="horizontal"==c.path.toString().toLowerCase()?{left:parseInt(d.width/2-b.loader.outerWidth()/2)}:{top:parseInt(d.height/2-b.loader.outerHeight()/2)};b.loader.addClass(c.skin).css(d);b.nextButton.add(b.prevButton).addClass(c.skin);"horizontal"==e&&b.loader.add(b.nextButton).add(b.prevButton).addClass("horizontal");b.BODY[b.isMobile?"addClass":"removeClass"]("isMobile");c.infinite||(b.prevButton.add(b.prevButton).add(b.innerPrevButton).add(b.innerNextButton).removeClass("disabled"),
...
function(c){c&&g.extend(!0,e,{type:c.type,options:{html5video:c.html5video,width:"image"==c.type?0:c.width||e.width,height:"image"==c.type?0:c.height||e.height,thumbnail:e.options.thumbnail||c.thumbnail}});f--;0==f&&(b.dontGenerateThumbs=!1,a.generateThumbnails())})});a.createUI();u.lightbox={close:function(){a.closeAction()},fullscreen:function(){a.fullScreenAction()},moveNext:function(){a.moveTo("next")},movePrev:function(){a.moveTo("prev")},goTo:function(b){a.goTo(b)},refresh:function(){a.refresh()},
reposition:function(){0<arguments.length?a.repositionPhoto(!0):a.repositionPhoto()},setOption:function(b){a.setOption(b)},destroy:function(){a.closeAction();a.dispatchItemsEvents()}};c.linkId&&(b.hashLock=!0,u.location.hash=c.linkId+"/"+b.current,setTimeout(function(){b.hashLock=!1},55));c.slideshow.startPaused||(a.resume(),b.innerPlayButton.removeClass("lightbox-play").addClass("lightbox-pause"));"function"==typeof a.options.callback.onOpen&&a.options.callback.onOpen.call(a)},loadContent:function(a,
b,c){var d=this,e,f;d.createUI();a.speed=c||d.options.effects.loadedFadeSpeed;"current"==b&&(d.vars.lockWheel=a.options.mousewheel?!1:!0,d.vars.lockSwipe=a.options.swipe?!1:!0);switch(b){case "current":e=d.vars.holder;f=d.vars.current;break;case "next":e=d.vars.nextPhoto;f=d.vars.next;break;case "prev":e=d.vars.prevPhoto,f=d.vars.prev}e.removeAttr("style class").addClass("lightbox-holder").addClass(a.options.skin);g("div.lightbox-inner-toolbar",e).remove();if(a.title||d.options.innerToolbar){c=
...
typeof f.callback.onAfterLoad&&f.callback.onAfterLoad.call(e,e.ui,c);"function"==typeof a.options.onAfterLoad&&a.options.onAfterLoad.call(e,h);b.data({naturalWidth:l?l.width:400,naturalHeight:l?l.height:200});g("div.lightbox-container",b).empty().append(l?'<img src="'+a.URL+'" class="lightbox-image" />':'<span class="lightbox-alert">'+f.errors.loadImage+"</span>");"function"==typeof f.callback.onRender&&f.callback.onRender.call(e,e.ui,c);"function"==typeof a.options.onRender&&a.options.onRender.call(e,
...
f.ajaxSetup.username,password:n.password||f.ajaxSetup.password,beforeSend:n.beforeSend||f.ajaxSetup.beforeSend,complete:n.complete||f.ajaxSetup.complete,success:function(l,k,m){e.hideLoader();var s=g(l),y=g("div.lightbox-container",b),u=e.items[c].options.width||parseInt(s.attr("width")),p=e.items[c].options.height||parseInt(s.attr("height")),v=s.attr("width")&&s.attr("height")?{overflow:"hidden"}:{};y.empty().append(g('<div class="lightbox-wrapper"></div>').css(v).html(s));b.show().data({naturalWidth:u||
...
k,m){"function"==typeof f.callback.onAfterLoad&&f.callback.onAfterLoad.call(e,e.ui,c);"function"==typeof a.options.onAfterLoad&&a.options.onAfterLoad.call(e,h);e.hideLoader();g("div.lightbox-container",b).empty().append('<span class="lightbox-alert">'+f.errors.loadContents+"</span>");e.configureHolder(a,d,b);f.ajaxSetup.error(l,k,m);"function"==typeof n.error&&n.error(l,k,m)}});break;case "html":k=a.URL;container=g("div.lightbox-container",b);k[0].nodeName?l=k.clone():(k=g(k),l=k.selector?g("<div>"+
...
a[e]})):b(!1)},patchItemsEvents:function(){var a=this,b=a.vars,c=D?"itap.lightbox":"click.lightbox",d=D?"click.lightbox":"itap.lightbox";g.each(a.itemsObject,function(e,f){f.on(c,function(){b.current=e;b.next=a.items[e+1]?e+1:null;b.prev=a.items[e-1]?e-1:null;a.addContents();a.patchEvents();return!1}).on(d,function(){return!1})})},dispatchItemsEvents:function(){g.each(this.itemsObject,function(a,b){b.off(".lightbox")})},refresh:function(){this.dispatchItemsEvents();this.attachItems();this.normalizeItems();
this.patchItemsEvents()},patchEvents:function(){function a(a){"mousemove"!==a.type||c.isMobile||(c.mouseID||c.hideableElements.show(),c.mouseID=clearTimeout(c.mouseID),c.mouseID=setTimeout(function(){c.hideableElements.hide();c.mouseID=clearTimeout(c.mouseID)},3E3))}var b=this,c=b.vars,d=b.options,e=d.path.toString().toLowerCase(),f=g(".lightbox-holder"),h=v.fullScreenEventName+".lightbox",l=verticalDistanceThreshold=100;M.bind("resize.lightbox",function(){var a=G();d.mobileOptimizer&&!d.innerToolbar&&(c.isMobile=
a.width<=c.mobileMaxWidth);c.BODY[c.isMobile?"addClass":"removeClass"]("isMobile");b.repositionPhoto(null);D&&(clearTimeout(c.setTime),c.setTime=setTimeout(function(){var a=K().y;u.scrollTo(0,a-30);u.scrollTo(0,a+30);u.scrollTo(0,a)},2E3));c.thumbs&&b.positionThumbnails()}).bind("keydown.lightbox",function(a){if(d.controls.keyboard)switch(a.keyCode){case 13:a.shiftKey&&d.keyboard.shift_enter&&b.fullScreenAction();break;case 27:d.keyboard.esc&&b.closeAction();break;case 37:d.keyboard.left&&!c.lockKey&&
b.moveTo("prev");break;case 38:d.keyboard.up&&!c.lockKey&&b.moveTo("prev");break;case 39:d.keyboard.right&&!c.lockKey&&b.moveTo("next");break;case 40:d.keyboard.down&&!c.lockKey&&b.moveTo("next")}});v.supportsFullScreen&&M.bind(h,function(){b.doFullscreen()});var h=[d.caption.show+".lightbox",d.caption.hide+".lightbox",d.social.show+".lightbox",d.social.hide+".lightbox"].filter(function(a,b,c){return c.lastIndexOf(a)===b}),k="";g.each(h,function(a,b){0!=a&&(k+=" ");k+=b});A.on(J,".lightbox-overlay",
function(){d.overlay.blur&&b.closeAction()}).on(J,".lightbox-next, .lightbox-next-button",function(){b.moveTo("next")}).on(J,".lightbox-prev, .lightbox-prev-button",function(){b.moveTo("prev")}).on(J,".lightbox-thumbnail",function(){var a=g(this),a=g(".lightbox-thumbnail",c.thumbnails).index(a);a!=c.current&&b.goTo(a)}).on(k,".lightbox-holder:not(.lightbox-next, .lightbox-prev)",function(a){var b=g("div.lightbox-caption",c.holder),e=g("div.lightbox-social",c.holder),f=d.effects.fadeSpeed;
c.nextLock||c.prevLock?(a.type!=d.caption.show||b.is(":visible")?a.type==d.caption.hide&&b.is(":visible")&&b.fadeOut(f):b.fadeIn(f),a.type!=d.social.show||e.is(":visible")?a.type==d.social.hide&&e.is(":visible")&&e.fadeOut(f):e.fadeIn(f)):(a.type!=d.caption.show||b.is(":visible")?a.type==d.caption.hide&&b.is(":visible")&&b.stop().fadeOut(f):b.stop().fadeIn(f),a.type!=d.social.show||e.is(":visible")?a.type==d.social.hide&&e.is(":visible")&&e.stop().fadeOut(f):e.stop().fadeIn(f))}).on("mouseenter.lightbox mouseleave.lightbox",
".lightbox-wrapper",function(a){c.lockWheel="mouseenter"==a.type?!0:!1}).on(J,".lightbox-toolbar a.lightbox-close, .lightbox-toolbar a.lightbox-fullscreen, .lightbox-toolbar a.lightbox-play, .lightbox-toolbar a.lightbox-pause",function(){var a=g(this);a.hasClass("lightbox-fullscreen")?b.fullScreenAction():a.hasClass("lightbox-play")?(b.resume(),a.addClass("lightbox-pause").removeClass("lightbox-play")):a.hasClass("lightbox-pause")?(b.pause(),a.addClass("lightbox-play").removeClass("lightbox-pause")):
b.closeAction()}).on(T,".lightbox-overlay, .lightbox-thumbnails-container",function(a){a.preventDefault()});if(d.controls.arrows&&!D)A.on("mousemove.lightbox",a);if(d.controls.slideshow&&d.slideshow.pauseOnHover)A.on("mouseenter.lightbox mouseleave.lightbox",".lightbox-holder:not(.lightbox-next, .lightbox-prev)",function(a){"mouseenter"==a.type&&c.cycleID?b.pause():"mouseleave"==a.type&&c.isPaused&&b.resume()});h=g(".lightbox-overlay, .lightbox-holder, .lightbox-thumbnails");if(d.controls.mousewheel)h.on("mousewheel.lightbox",
...
d[1]})});a.preventDefault()}}function k(){f.each(function(){var a=g(this),b=a.data("offset")||{top:a.offset().top-s,left:a.offset().left-q},c=b.top,b=b.left;a.css(z,F).stop().animate({top:c,left:b},500,"easeOutCirc",function(){a.css(z,"")})})}if(!c.nextLock&&!c.prevLock&&1!=c.total&&!c.lockSwipe){c.BODY.addClass("lightbox-closedhand");a=a.originalEvent.touches?a.originalEvent.touches[0]:a;var s=A.scrollTop(),q=A.scrollLeft(),r={time:(new Date).getTime(),coords:[a.pageX-q,a.pageY-s]},t;f.bind(T,h);
A.one(ka,function(a){f.unbind(T,h);c.BODY.removeClass("lightbox-closedhand");r&&t&&("horizontal"==e&&1E3>t.time-r.time&&Math.abs(r.coords[0]-t.coords[0])>l&&Math.abs(r.coords[1]-t.coords[1])<verticalDistanceThreshold?r.coords[0]>t.coords[0]?c.current!=c.total-1||d.infinite?(c.isSwipe=!0,b.moveTo("next")):k():0!=c.current||d.infinite?(c.isSwipe=!0,b.moveTo("prev")):k():"vertical"==e&&1E3>t.time-r.time&&Math.abs(r.coords[1]-t.coords[1])>l&&Math.abs(r.coords[0]-t.coords[0])<verticalDistanceThreshold?
...
(c.hashLock=!0,u.location.hash=d.linkId+"/"+a);b.items[a]&&(b.items[a].options.mousewheel?b.vars.lockWheel=!1:c.lockWheel=!0,c.lockSwipe=b.items[a].options.swipe?!1:!0);g.each([c.holder,c.nextPhoto,c.prevPhoto],function(a,b){b.css(z,F).fadeOut(d.effects.loadedFadeSpeed)});c.current=a;c.next=a+1;c.prev=a-1;b.createUI();setTimeout(function(){b.generateBoxes()},d.effects.loadedFadeSpeed+50);g(".lightbox-thumbnail",c.thumbnails).removeClass("lightbox-active").eq(a).addClass("lightbox-active");b.positionThumbnails();
d.linkId&&setTimeout(function(){c.hashLock=!1},55);d.infinite||(c.nextButton.add(c.prevButton).add(c.innerPrevButton).add(c.innerNextButton).removeClass("disabled"),0==c.current&&c.prevButton.add(c.innerPrevButton).addClass("disabled"),c.current>=c.total-1&&c.nextButton.add(c.innerNextButton).addClass("disabled"));b.resetCycle();"function"==typeof d.callback.onAfterChange&&d.callback.onAfterChange.call(b,b.ui)}},moveTo:function(a){var b=this,c=b.vars,d=b.options,e=d.path.toString().toLowerCase(),f=G(),h=d.effects.switchSpeed;
if(c.nextLock||c.prevLock)return!1;var l="next"==a?c.next:c.prev;d.linkId&&(c.hashLock=!0,u.location.hash=d.linkId+"/"+l);if("next"==a){if(!b.items[l])return!1;var k=c.nextPhoto,m=c.holder,n=c.prevPhoto,y="lightbox-prev",s="lightbox-next"}else if("prev"==a){if(!b.items[l])return!1;k=c.prevPhoto;m=c.holder;n=c.nextPhoto;y="lightbox-next";s="lightbox-prev"}"function"==typeof d.callback.onBeforeChange&&d.callback.onBeforeChange.call(b,b.ui);"next"==a?c.nextLock=!0:c.prevLock=!0;var q=g("div.lightbox-caption",
m),r=g("div.lightbox-social",m);q.length&&q.stop().fadeOut(h,function(){g(this).remove()});r.length&&r.stop().fadeOut(h,function(){g(this).remove()});b.items[l].caption&&(b.setCaption(b.items[l],k),q=g("div.lightbox-caption",k),r=parseInt(q.outerHeight()/k.outerHeight()*100),d.caption.start&&50>=r&&q.fadeIn(h));if(q=b.items[l].options.social)b.setSocial(q,b.items[l].URL,k),d.social.start&&g("div.lightbox-social",k).fadeIn(d.effects.fadeSpeed);g.each([k,m,n],function(a,b){b.removeClass("lightbox-next lightbox-prev")});
var t=k.data("offset"),q=f.width-d.styles.pageOffsetX,f=f.height-d.styles.pageOffsetY,r=t.newDims.width,p=t.newDims.height,v=t.thumbsOffset,t=t.diff,w=parseInt(f/2-p/2-t.H-v.H/2),t=parseInt(q/2-r/2-t.W-v.W/2);k.css(z,F).animate({top:w,left:t,opacity:1},h,c.isSwipe?"easeOutCirc":"easeInOutCirc",function(){k.css(z,"")});g("div.lightbox-container",k).animate({width:r,height:p},h,c.isSwipe?"easeOutCirc":"easeInOutCirc");var p=m.data("offset"),x=p.object,t=p.diff,r=p.newDims.width,p=p.newDims.height,
r=parseInt(r*d.styles["next"==a?"prevScale":"nextScale"]),p=parseInt(p*d.styles["next"==a?"prevScale":"nextScale"]),w="horizontal"==e?parseInt(f/2-x.offsetY-p/2-t.H-v.H/2):parseInt(f-x.offsetX-t.H-v.H/2);"prev"==a?t="horizontal"==e?parseInt(q-x.offsetX-t.W-v.W/2):parseInt(q/2-r/2-t.W-x.offsetY-v.W/2):(w="horizontal"==e?w:parseInt(x.offsetX-t.H-p-v.H/2),t="horizontal"==e?parseInt(x.offsetX-t.W-r-v.W/2):parseInt(q/2-x.offsetY-r/2-t.W-v.W/2));g("div.lightbox-container",m).animate({width:r,height:p},
h,c.isSwipe?"easeOutCirc":"easeInOutCirc");m.addClass(y).css(z,F).animate({top:w,left:t,opacity:d.styles.prevOpacity},h,c.isSwipe?"easeOutCirc":"easeInOutCirc",function(){m.css(z,"");g(".lightbox-thumbnail",c.thumbnails).removeClass("lightbox-active").eq(l).addClass("lightbox-active");b.positionThumbnails();b.items[l]&&(c.lockWheel=b.items[l].options.mousewheel?!1:!0,c.lockSwipe=b.items[l].options.swipe?!1:!0);c.isSwipe=!1;"next"==a?(c.nextPhoto=n,c.prevPhoto=m,c.holder=k,c.nextPhoto.hide(),c.next+=
...
parseInt(2*w);t="horizontal"==e?"next"==a?parseInt(-(q/2)-n.outerWidth()):parseInt(2*t):C(n,"left");n.css(z,F).animate({top:w,left:t,opacity:d.styles.nextOpacity},h,c.isSwipe?"easeOutCirc":"easeInOutCirc",function(){n.css(z,"")}).addClass(s)},setCaption:function(a,b){var c=g('<div class="lightbox-caption"></div>');a.caption&&(c.html(a.caption),g("div.lightbox-container",b).append(c))},normalizeSocial:function(a,b){var c=this.options,d=u.location.href;g.each(a,function(e,f){var h,l;switch(e.toString().toLowerCase()){case "facebook":h=
...
g.each(b,function(a,b){b.show()}),a.fullScreenButton.attr("title",c.text.enterFullscreen),d.data({naturalWidth:d.data("naturalWidthOld"),naturalHeight:d.data("naturalHeightOld"),naturalWidthOld:null,naturalHeightOld:null}),g.each(l,function(a,b){b.removeClass("lightbox-fullscreen")}),"function"==typeof c.callback.onExitFullScreen&&c.callback.onExitFullScreen.call(this,this.ui);else{a.isInFullScreen=a.lockKey=a.lockWheel=a.lockSwipe=!0;a.overlay.css({opacity:1});g.each(b,function(a,b){b.hide()});
a.fullScreenButton.attr("title",c.text.exitFullscreen);if(-1!=c.fullStretchTypes.indexOf(e.type))d.data({naturalWidthOld:d.data("naturalWidth"),naturalHeightOld:d.data("naturalHeight"),naturalWidth:f,naturalHeight:h});else{var b=e.options.fullViewPort||c.fullViewPort||"",a=f,e=h,f=d.data("naturalWidth"),k=d.data("naturalHeight");"fill"==b.toString().toLowerCase()?(e=a/f*k,e<h&&(a=h/k*f,e=h)):"fit"==b.toString().toLowerCase()?(h=this.getNewDimenstions(a,e,f,k,!0),a=h.width,e=h.height):"stretch"!=b.toString().toLowerCase()&&(h=this.getNewDimenstions(a,
e,f,k,f>a||k>e?!0:!1),a=h.width,e=h.height);d.data({naturalWidthOld:d.data("naturalWidth"),naturalHeightOld:d.data("naturalHeight"),naturalWidth:a,naturalHeight:e})}g.each(l,function(a,b){b.addClass("lightbox-fullscreen")});"function"==typeof c.callback.onEnterFullScreen&&c.callback.onEnterFullScreen.call(this,this.ui)}}else a.isInFullScreen=a.isInFullScreen?!1:!0;this.repositionPhoto(!0)},closeAction:function(){var a=this.vars,b=this.options;M.unbind(".lightbox");a.isInFullScreen&&v.cancelFullScreen(document.documentElement);
A.off(".lightbox");g(".lightbox-overlay, .lightbox-holder, .lightbox-thumbnails").off(".lightbox");b.hide.effect?a.overlay.stop().fadeOut(b.hide.speed,function(){a.overlay.remove();a.BODY.removeClass("lightbox-noscroll").off(".lightbox")}):(a.overlay.remove(),a.BODY.removeClass("lightbox-noscroll").off(".lightbox"));g.each([a.toolbar,a.holder,a.nextPhoto,a.prevPhoto,a.nextButton,a.prevButton,a.loader,a.thumbnails],function(a,b){b.removeAttr("style").remove()});a.dontGenerateThumbs=a.isInFullScreen=
!1;u.lightbox=null;b.linkId&&(a.hashLock=!0,da(),setTimeout(function(){a.hashLock=!1},55));"function"==typeof b.callback.onHide&&b.callback.onHide.call(this,this.ui)},repositionPhoto:function(){var a=this.vars,b=this.options,c=b.path.toString().toLowerCase(),d=G(),e=d.width,f=d.height,d=a.isInFullScreen&&b.fullAlone||a.isMobile?0:"horizontal"==c?0:a.thumbnails.outerWidth(),h=a.isMobile?a.toolbar.outerHeight():a.isInFullScreen&&b.fullAlone?0:"horizontal"==c?a.thumbnails.outerHeight():0,e=a.isInFullScreen&&b.fullAlone?
...
2)};a.loader.css(b)},repositionEl:function(a){var b=this.vars,c=this.options,d=c.path.toString().toLowerCase(),e="current"==a.type?b.isInFullScreen&&c.fullAlone?a.width:a.width-a.offsetW:a.width-a.offsetW,f="current"==a.type?b.isInFullScreen&&c.fullAlone?a.height:a.height-a.offsetH:a.height-a.offsetH,h=a.item,l=a.item.options,k=a.holder,m=a.offsetX||0,n=a.offsetY||0,p=a.thumbsOffsetW,s=a.thumbsOffsetH;"current"==a.type?("number"==typeof l.width&&l.width&&(e=b.isInFullScreen&&c.fullAlone&&(-1!=c.fullStretchTypes.indexOf(h.type)||
l.fullViewPort||c.fullViewPort)?e:l.width>e?e:l.width),"number"==typeof l.height&&l.height&&(f=b.isInFullScreen&&c.fullAlone&&(-1!=c.fullStretchTypes.indexOf(h.type)||l.fullViewPort||c.fullViewPort)?f:l.height>f?f:l.height)):("number"==typeof l.width&&l.width&&(e=l.width>e?e:l.width),"number"==typeof l.height&&l.height&&(f=l.height>f?f:l.height));f=parseInt(f-g(".lightbox-inner-toolbar",k).outerHeight());b="string"==typeof l.width&&-1!=l.width.indexOf("%")?Y(parseInt(l.width.replace("%","")),a.width):
...
(b=h.width,h=h.height);f=parseInt((C(k,"padding-left")+C(k,"padding-right")+C(k,"border-left-width")+C(k,"border-right-width"))/2);l=parseInt((C(k,"padding-top")+C(k,"padding-bottom")+C(k,"border-top-width")+C(k,"border-bottom-width")+g(".lightbox-inner-toolbar",k).outerHeight())/2);switch(a.type){case "current":var q=parseInt(a.height/2-h/2-l-s/2),r=parseInt(a.width/2-b/2-f-p/2);break;case "next":q="horizontal"==d?parseInt(a.height/2-n-h/2-l-s/2):parseInt(a.height-m-l-s/2);r="horizontal"==d?parseInt(a.width-
m-f-p/2):parseInt(a.width/2-b/2-f-n-p/2);break;case "prev":q="horizontal"==d?parseInt(a.height/2-n-h/2-l-s/2):parseInt(m-l-h-s/2),r="horizontal"==d?parseInt(m-f-b-p/2):parseInt(a.width/2-n-b/2-f-p/2)}k.data("offset",{top:q,left:r,newDims:e,diff:{W:f,H:l},thumbsOffset:{W:p,H:s},object:a});0<a.animate&&c.effects.reposition?(k.css(z,F).stop().animate({top:q,left:r},c.effects.repositionSpeed,"easeOutCirc",function(){k.css(z,"")}),g("div.lightbox-container",k).stop().animate({width:b,height:h},c.effects.repositionSpeed,
"easeOutCirc"),g("div.lightbox-inner-toolbar",k).stop().animate({width:b},c.effects.repositionSpeed,"easeOutCirc",function(){g(this).css("overflow","visible")})):(k.css({top:q,left:r}),g("div.lightbox-container",k).css({width:b,height:h}),g("div.lightbox-inner-toolbar",k).css({width:b}))},resume:function(a){var b=this,c=b.vars,d=b.options;!d.slideshow.pauseTime||d.controls.slideshow&&1>=c.total||a<c.isPaused||(c.isPaused=0,c.cycleID&&(c.cycleID=clearTimeout(c.cycleID)),c.cycleID=setTimeout(function(){c.current==
...
{src:b.URL,type:d})):this.plugins.quicktime?(c=g("<object />",{type:"video/quicktime",pluginspage:"http://www.apple.com/quicktime/download"}).attr({data:b.URL,width:"100%",height:"100%"}).append(g("<param />",{name:"src",value:b.URL})).append(g("<param />",{name:"autoplay",value:"false"})).append(g("<param />",{name:"loop",value:"false"})).append(g("<param />",{name:"scale",value:"tofit"})),x.msie&&(c=ha(b.URL,"100%","100%","","SCALE","tofit","AUTOPLAY","false","LOOP","false"))):c=g("<span />",{"class":"lightbox-alert",
...
"100%",height:"number"==typeof b.options.height&&b.options.height&&"1"==this.options.minScale&&"1"==this.options.maxScale?b.options.height:"100%",quality:"high",bgcolor:"#000000",play:"true",loop:"true",menu:"true",wmode:"transparent",scale:"showall",allowScriptAccess:"always",allowFullScreen:"true",flashvars:f,fullscreen:"yes"})}else c=g("<span />",{"class":"lightbox-alert",html:this.options.errors.missingPlugin.replace("{pluginspage}","http://www.adobe.com/go/getflash").replace("{type}","Adobe Flash player")});
break;case "iframe":c=g("<iframe />").attr({width:"number"==typeof b.options.width&&b.options.width&&"1"==this.options.minScale&&"1"==this.options.maxScale?b.options.width:"100%",height:"number"==typeof b.options.height&&b.options.height&&"1"==this.options.minScale&&"1"==this.options.maxScale?b.options.height:"100%",src:b.URL,frameborder:0,webkitAllowFullScreen:"",mozallowfullscreen:"",allowFullScreen:""});break;case "inline":c=g('<div class="lightbox-wrapper"></div>').html(g(b.URL).clone(!0));break;
case "html":c=b.URL,c[0].nodeName||(c=g(b.URL),c=c.selector?g("<div>"+c+"</div>"):c),c=g('<div class="lightbox-wrapper"></div>').html(c)}g("div.lightbox-container",a).empty().html(c);return c},ogpRecognition:function(a,b){var c=this,d=a.URL,e={length:!1};c.showLoader();ea(d,function(a){c.hideLoader();if(200==a.status){a=a.results;var d=a.type,l=a.source;e.source=l.src;e.width=l.width&&parseInt(l.width)||0;e.height=l.height&&parseInt(l.height)||0;e.type=d;e.thumbnail=l.thumbnail||a.images[0];e.html5video=
...
(a=g(".lightbox-overlay"),a.length&&a.attr("linkid")==c.linkId?this.goTo(b):this.itemsObject[b].trigger("click")):(a=g(".lightbox-overlay"),a.length&&this.closeAction())):(a=g(".lightbox-overlay"),a.length&&this.closeAction()))}};g.fn.lightbox=function(){var a=arguments,b=g.isPlainObject(a[0])?a[0]:a[1],c=g.isArray(a[0])||"string"==typeof a[0]?a[0]:a[1];b||(b={});var b=g.extend(!0,{attr:"href",path:"vertical",skin:"dark",linkId:!1,infinite:!1,startFrom:0,randomStart:!1,keepAspectRatio:!0,maxScale:1,
...
destroy:function(){e.closeAction();e.dispatchItemsEvents()}}}throw"The jQuery version that was loaded is too old. lightbox requires jQuery 1.8+";};g.lightbox=function(a,b){return g.fn.lightbox(a,b)};g.extend(g.easing,{easeInCirc:function(a,b,c,d,e){return-d*(Math.sqrt(1-(b/=e)*b)-1)+c},easeOutCirc:function(a,b,c,d,e){return d*Math.sqrt(1-(b=b/e-1)*b)+c},easeInOutCirc:function(a,b,c,d,e){return 1>(b/=e/2)?-d/2*(Math.sqrt(1-b*b)-1)+c:d/2*(Math.sqrt(1-(b-=2)*b)+1)+c}});g(document);g.each("touchstart touchmove touchend tap taphold swipe swipeleft swiperight scrollstart scrollstop".split(" "),
...
(this.isOpera=/Opera\s*[\/]?\s*(\d+\.?\d*)/i.test(d))&&(/Version\s*\/\s*(\d+\.?\d*)/i.test(d)||1)?parseFloat(RegExp.$1,10):null;this.addWinEvent("load",this.handler(this.runWLfuncs,this))},init:function(a){var b,c={status:-3,plugin:0};if(!this.isString(a))return c;if(1==a.length)return this.getVersionDelimiter=a,c;a=a.toString().toLowerCase().replace(/\s/g,"");b=this.Plugins[a];if(!b||!b.getVersion)return c;c.plugin=b;this.isDefined(b.installed)||(b.installed=null,b.version=null,b.version0=null,b.getVersionDone=
...
jQuery(document).ready(function($){
	var galleries = $('.lightbox_gallery'),
	...
	if(galleries.length){
		galleries.each(function(){
			...
			kid.lightbox(options);
		});
	}

	if(jetpackGalleries.length && lightbox.jetPack){
		jetpackGalleries.each(function(){
			var t = $(this),
			kid = $('a', t),
			options = lightbox.options && eval("(" + rawurldecode(lightbox.options) + ")") || {};
			options.attr = 'data-source';
			...
			kid.each(function(i){
				...
				$this.attr('data-source', origFile);
			});
	...
	if(nextGenGalleries.length && lightbox.nextGEN){
	...
	$(document).on('click', '.lightbox_inline_gallery', function(){
		var t = $(this),
