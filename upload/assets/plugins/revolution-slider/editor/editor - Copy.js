...
(function( $ ) {
	...
	$.fn.revoSliderEditor = function( initvar ) {
		...
		this.process = function( siteURL , adminURL, delayTime ) {
			...
			$('input, select ,textarea', '#slider_form').change( function(){  
				if( $(this).attr('name') =='layer_top' || $(this).attr('name') == 'layer_left') {  
					$this.currentLayer.css( { top:$('[name="layer_top"]','#slider_form').val()+"px",			
					 						  left:$('[name="layer_left"]','#slider_form').val()+"px"				
					 						});	
				}
				$this.state=true;
				$this.storeCurrentLayerData();  
			...
			$.ajax({
				...
				success: function(html_data) {
					...
					$(document).on('click', '.tp-typo',function() {	
						...
						if(layer_class.length){
							//console.log(layer_class);
							$('#'+layer_id).removeClass(layer_class);
						}
						if(new_class.length){
						...
						$('#'+field).val(new_class);
						if( $("#"+field).val()) { 
							$this.currentLayer.removeClass(layer_class).addClass(new_class);	
							$this.storeCurrentLayerData(); 
						}
		...
		this.createDefaultLayerData = function( layer, data ){
	 		...
	 		if( data !=null && data ) { 
				...
				data['layer_caption'] = data['layer_caption'].replace(/_apos_/,'\'');
				 ...
				 $('[name="layer_time['+data['layer_id']+']"]','#slider_form').val( data['time_start'] );
				...
				$('[name="layer_speed"]','#slider_form').val(  350 );
				$('[name="layer_left"]','#slider_form').val(  0 );
				$('[name="layer_top"]','#slider_form').val(  0 );
				$('[name="layer_class"]','#slider_form').val(  '');
				$('[name="layer_speed"]','#slider_form').val(  350 );
				$('[name="layer_endtime"]','#slider_form').val(  0 );
				$('[name="layer_endspeed"]','#slider_form').val(  300 );
				$('[name="layer_endanimation"]','#slider_form').val(  'auto');
				$('[name="layer_endeasing"]','#slider_form').val('nothing');
		...
		this.updatePosition = function( left, top ){
			 $('[name="layer_top"]','#slider_form').val( parseInt(top) );
			 $('[name="layer_left"]','#slider_form').val( parseInt(left) );
