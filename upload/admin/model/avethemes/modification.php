...
*******************************************************/
class ModelAvethemesModification extends Model {
	public function addModification($data) {
	$sql = "INSERT INTO " . DB_PREFIX . "modification SET `code` = '" . $this->db->escape($data['code']) . "', `name` = '" . $this->db->escape($data['name']) . "', 
	`author` = '" . $this->db->escape($data['author']) . "', `version` = '" . $this->db->escape($data['version']) . "', `link` = '" . $this->db->escape($data['link']) . "', `xml` = '" . $this->db->escape(html_entity_decode($data['xml'])) . "', `status` = '" . (int)$data['status'] . "', `date_added` = NOW()";
		$this->db->query($sql);
	}

	public function editModification($modification_id, $data) {	
		$sql = "UPDATE " . DB_PREFIX . "modification SET `code` = '" . $this->db->escape($data['code']) . "', `name` = '" . $this->db->escape($data['name']) . "', 
	`author` = '" . $this->db->escape($data['author']) . "', `version` = '" . $this->db->escape($data['version']) . "', `link` = '" . $this->db->escape($data['link']) . "', `xml` = '" . $this->db->escape(html_entity_decode($data['xml'])) . "', `status` = '" . (int)$data['status'] . "' WHERE modification_id = '" . (int)$modification_id . "'";	
		$this->db->query($sql);
	}
