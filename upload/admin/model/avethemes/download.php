...
*******************************************************/
class ModelAvethemesDownload extends Model {
	public function addDownload($data) {
      	$this->db->query("INSERT INTO ".DB_PREFIX."ave_download SET 
		filename = '" . $this->db->escape($data['filename']) . "',
		 mask = '" . $this->db->escape($data['mask']) . "', 
		`color` = '".$this->db->escape($data['color'])."', 
		 auth_key = '" . $this->db->escape($data['auth_key']) . "',
		 date_added = NOW()");
	...
	public function editDownload($download_id, $data) {		
        $this->db->query("UPDATE ".DB_PREFIX."ave_download SET 
		filename = '" . $this->db->escape($data['filename']) . "', 
		mask = '" . $this->db->escape($data['mask']) . "', 
		`color` = '".$this->db->escape($data['color'])."', 
		auth_key = '" . $this->db->escape($data['auth_key']) . "'
		WHERE download_id = '" . (int)$download_id . "'");
      	...
      	$this->db->query("DELETE FROM ".DB_PREFIX."ave_download_description WHERE download_id = '" . (int)$download_id . "'");
	...
	public function deleteDownload($download_id) {
		$query = $this->db->query("SELECT filename FROM ".DB_PREFIX."ave_download WHERE download_id = '" . (int)$download_id . "'");
		$filename=$query->row['filename'];
		if (file_exists(DIR_DOWNLOAD.$filename)) {
		unlink(DIR_DOWNLOAD.$filename);	
		}
      	$this->db->query("DELETE FROM ".DB_PREFIX."ave_download WHERE download_id = '" . (int)$download_id . "'");
	  	$this->db->query("DELETE FROM ".DB_PREFIX."ave_download_description WHERE download_id = '" . (int)$download_id . "'");	
	...
	public function getDownloadDescriptions($download_id) {
		...
		$query = $this->db->query("SELECT * FROM ".DB_PREFIX."ave_download_description WHERE download_id = '" . (int)$download_id . "'");
	...
	public function getTotalDownloads() {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM ".DB_PREFIX."ave_download");
