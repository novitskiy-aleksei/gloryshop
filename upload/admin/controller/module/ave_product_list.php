...
class ControllerModuleAveProductList extends Controller {  
	...
	public function delete() {
		...
		if (isset($this->request->get['module_id']) && $this->user->hasPermission('modify', 'module/ave_product_list')) {
			$this->model_extension_module->deleteModule($this->request->get['module_id']);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->response->redirect($this->url->link('module/ave_product_list', 'token=' . $this->session->data['token'], 'SSL'));
		}

		$this->index();
	} 
	public function index() {
		$data['redirect'] = $this->url->link('module/ave_product_list', 'token=' . $this->session->data['token'], 'SSL');
		$data['import_module'] = $this->url->link('feed/visual_layout_builder/import_module', 'token=' . $this->session->data['token'], 'SSL');
		$data['export_module'] = $this->url->link('feed/visual_layout_builder/export_module', '&code=ave_product_list&token=' . $this->session->data['token'] , 'SSL');
		
		$language_data = $this->load->language('module/ave_product_list');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('ave_product_list', $this->request->post);
			} else {
		...
		$modules = $this->model_extension_module->getModulesByCode('ave_product_list');
		if(!empty($modules)){
					$thumb = false;
					$has_data = true;
					foreach ($modules as $module) {
							...
							$data['module_data'][$module['module_id']] = array(
								...
								'delete'      => $this->url->link('module/ave_product_list/delete', 'token=' . $this->session->data['token'] . '&module_id=' . $module['module_id'], 'SSL'),
								'href'      => $this->url->link('module/ave_product_list', 'token=' . $this->session->data['token'] . '&module_id=' . $module['module_id'], 'SSL')
							);
					}
		}	
  		...
  		$data['breadcrumbs'] = array();
   		...
   		$data['breadcrumbs'][] = array(
			...
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('module/ave_product_list', 'token=' . $this->session->data['token'], 'SSL')
			);
		if (isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				'text' => 'Module ID: '.$this->request->get['module_id'],
				'href' => $this->url->link('module/ave_product_list', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);			
		}

		if (!isset($this->request->get['module_id'])) {
			$data['module_id'] = 0;
			$data['action'] = $this->url->link('module/ave_product_list', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['module_id'] = $this->request->get['module_id'];
			$data['action'] = $this->url->link('module/ave_product_list', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		if(empty($data['parent_id'])){
			...
			$data['categories_group'] = $this->model_avethemes_global->getProductCategoryGroup($id_group);
		}
		...
		$this_template = 'avethemes/module/product_list.tpl';		
...
$data['shortcut_group'] = $this->load->controller('ave/shortcut/blog');
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/ave_product_list')) {
	...
	protected function elements() {
		$this->load->language('module/ave_product_list');
		$elements[] = array(
			'label'  => $this->language->get('text_tab_product_grid'),
			'value'  => 'product-tab-grid'
		);
		$elements[] = array(
			'label'  => $this->language->get('text_tab_product_grid_carousel'),
			'value'  => 'product-tab-grid-carousel'
		);
		$elements[] = array(
			'label'  => $this->language->get('text_tab_product_list'),
			'value'  => 'product-tab-list'
		);
		$elements[] = array(
			'label'  => $this->language->get('text_tab_product_list_carousel'),
		...
		$elements[] = array(
			...
			'value'  => 'product-list-carousel'
		);
		$elements[] = array(
			'label'  => $this->language->get('text_product_carousel_list_desc'),
			'value'  => 'product-list-carousel-desc'
		);
		return $elements;
	}
}
