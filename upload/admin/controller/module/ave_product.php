...
class ControllerModuleAveProduct extends Controller {  	
	...
	public function delete() {
		...
		if (isset($this->request->get['module_id']) && $this->user->hasPermission('modify', 'module/ave_product')) {
			$this->model_extension_module->deleteModule($this->request->get['module_id']);

			$this->session->data['success'] = $this->language->get('text_success');

			$this->response->redirect($this->url->link('module/ave_product', 'token=' . $this->session->data['token'], 'SSL'));
		}

		$this->index();
	} 
	public function index() {
		$data['redirect'] = $this->url->link('module/ave_product', 'token=' . $this->session->data['token'], 'SSL');
		$data['import_module'] = $this->url->link('feed/visual_layout_builder/import_module', 'token=' . $this->session->data['token'], 'SSL');
		$data['export_module'] = $this->url->link('feed/visual_layout_builder/export_module', '&code=ave_product&token=' . $this->session->data['token'] , 'SSL');
		
		$language_data = $this->load->language('module/ave_product');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_extension_module->addModule('ave_product', $this->request->post);
			} else {
		...
		$modules = $this->model_extension_module->getModulesByCode('ave_product');
		if(!empty($modules)){
					$thumb = false;
					$has_data = true;
					foreach ($modules as $module) {
							...
							$data['module_data'][$module['module_id']] = array(
								...
								'delete'      => $this->url->link('module/ave_product/delete', 'token=' . $this->session->data['token'] . '&module_id=' . $module['module_id'], 'SSL'),
								'href'      => $this->url->link('module/ave_product', 'token=' . $this->session->data['token'] . '&module_id=' . $module['module_id'], 'SSL')
							);
					}
		}	
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('module/ave_product', 'token=' . $this->session->data['token'], 'SSL')
		);
		if (isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				'text' => 'Module ID: '.$this->request->get['module_id'],
				'href' => $this->url->link('module/ave_product', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);			
		}

		if (!isset($this->request->get['module_id'])) {
			$data['module_id'] = 0;
			$data['action'] = $this->url->link('module/ave_product', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['module_id'] = $this->request->get['module_id'];
			$data['action'] = $this->url->link('module/ave_product', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		$this_template = 'avethemes/module/product_by_type.tpl';		
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/ave_product')) {
	...
	protected function elements() {
		$this->load->language('module/ave_product');
		$elements[] = array(
			'label'  => $this->language->get('text_product_carousel_grid'),
			'value'  => 'product-carousel-grid'
		);
		$elements[] = array(
			'label'  => $this->language->get('text_product_carousel_grid_flip'),
			'value'  => 'product-carousel-grid-flip'
		);
		$elements[] = array(
			'label'  => $this->language->get('text_product_carousel_list'),
			'value'  => 'product-carousel-list'
		);
		$elements[] = array(
		...
		$elements[] = array(
			...
			'value'  => 'product-sidebar'
		);
		$elements[] = array(
			'label'  => $this->language->get('text_sidebar_product_carousel'),
			'value'  => 'product-sidebar_carousel'
		);
		$elements[] = array(
		...
		$elements[] = array(
			'label'  => $this->language->get('text_small_carousel'),
			'value'  => 'product-carousel-small'
		);
		return $elements;
	}
}
