...
class ModelAvethemesSliderRevolution extends Model {	
	...
	public function getSliderGroupById( $id ){
		$query = ' SELECT * FROM '. DB_PREFIX . "ave_sliderrevo ";
		$query .= ' WHERE primary_id='.(int)$id;
		...
		$sliderGroup = $query->row;
	 	...
	 	if( $sliderGroup ){
			$sliderGroup['configs'] = unserialize($sliderGroup['configs'] );
			$sliderGroup['configs'] = array_merge($configs, $sliderGroup['configs'] );
		}else {
			$sliderGroup['configs'] = $configs;
		}
	...
	public function getSlidersByGroupId( $groupID, $languageID ){
		...
		$query .= ' WHERE primary_id='.(int)$groupID .' AND `language_id`='.(int)$languageID.' AND `status` = 1 ORDER BY position ASC';
